 ...
 */
ow_err_t ow_xfer(ow_t *handle, uint8_t fn_cmd, const uint8_t *w_data, uint16_t w_len,
                 uint16_t r_len)
{
    assert_param(handle != NULL);
 ...
 * @retval Error code (ow_err_t).
 */
ow_err_t ow_xfer_by_id(ow_t *handle, uint8_t rom_id, uint8_t fn_cmd, const uint8_t *w_data,
                       uint16_t w_len, uint16_t r_len)
{
    assert_param(handle != NULL);
...
__STATIC_FORCEINLINE void ow_state_xfer(ow_t *handle)
{
    ...
    case 3:
        __HAL_TIM_SET_AUTORELOAD(
            handle->config.tim_handle,
            (handle->buf.data[handle->buf.byte_idx] & (1 << handle->buf.bit_idx))
                ? OW_TIM_WRITE_LOW - 1
                : OW_TIM_WRITE_HIGH - 1);
        ow_write_bit(handle, false);
        handle->buf.bit_ph++;
    ...
    case 7:
        ...
        if (ow_read_bit(handle)) {
            handle->buf.data[handle->buf.write_len + handle->buf.byte_idx] |=
                (1 << handle->buf.bit_idx);
        }
    ...
    case 10:
        ...
        if (handle->search.val == OW_VAL_DIFF) {
            ...
            if (bit_number < handle->search.last_discrepancy) {
                ...
                bit_choice =
                    (handle->search.rom_id[handle->buf.bit_idx / 8] >> (handle->buf.bit_idx % 8))
                    & 0x01;
